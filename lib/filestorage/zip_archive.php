...
/* Conditionally load the externalzip plugin resources (if plugin is present.)
 * Otherwise define the constant used within patched /lib/filestorage/zip_archive.php.
 * This allows the backup plugin to work with no warnings if externalzip is not installed.
 *
 * @link https://tracker.moodle.org/browse/MDL-34388
 */
if (file_exists("{$CFG->dirroot}/local/externalzip")) {
    require_once("{$CFG->dirroot}/local/externalzip/lib.php");
    require_once("{$CFG->dirroot}/local/externalzip/externalzip_archive.php");
} else {
    define('LOCAL_EXTERNALZIP_EXT', 'externalzip');
}
